 ...
 */
{
   ...
   */
  const SCOPED = /^\s*\.(slide-title|note|stat|mini|cbc|cbc-\w+|card-title|cb-\w+|lang-tab|clang-tab|brand-name|brand-sub|nav-label|nav-group|nav-item|ai-badge|rail-foot|subtitle|dates|btn|status|stage-name|stage-val|stage-note|seg-val|stage(?![\w-])|seg(?![\w-])|card(?=\s+h2)|mr-[\w-]+|legend|ihint|pill|all-four|tag|qtag|gap-tag|hcode|hcode-sm|af(?:head|row|share|pkg))\b|^\s*(?:table|h1|th)\{|^\s*tr\.grp\s|^\s*table\.sortable\s/;
  /**
